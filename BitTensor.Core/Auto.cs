...
public static class Auto
{
    ...
    public static GetGradientsFunction<T> Grad<T>(T output) where T : AbstractTensorNode<T>, ITensor<T>
    {
    ...
    public static Dictionary<T, T> GetGradients<T>(T output) where T : AbstractTensorNode<T>, ITensor<T>
    {
        if (!output.IsScalar)
        ...
        while (nodes.Count > 0)
        {
            ...
            for (var i = 0; i < children.Length; ++i)
            {
                ...
                {
                    grads[child] = T.Add(grads[child], grad);
                }
                else
                {
                    grads[child] = grad;
